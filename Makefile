...
OBJECTS += velvet utils collections vte text csi csi_dispatch screen osc dcs io velvet_scene velvet_input velvet_cmd velvet_lua velvet_api velvet_alloc
OBJECT_DIR = src
...
$(OUT_DIR)/%.c.o: $(OBJECT_DIR)/%.c $(GEN_LUA_AUTOGEN) | $(OUT_DIR) $(SUBMODULE_INIT)
	@echo $(CC) -c $(CFLAGS) $< -o $@ > $@.txt
	$(CC) -c $(CFLAGS) $< -o $@

$(OUT_DIR)/%.c.o: $(CMD_DIR)/%.c $(GEN_LUA_AUTOGEN) | $(OUT_DIR) $(SUBMODULE_INIT)
	@echo $(CC) -c $(CFLAGS) $< -o $@ > $@.txt
	$(CC) -c $(CFLAGS) $< -o $@
