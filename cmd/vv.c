...
static void app_detach_session(struct app_context *app, struct session *s) {
  ...
  vec_remove(&app->sessions, vec_index(s, app->sessions));
}
...
static void session_input_callback(struct io_source *src, struct u8_slice str) {
  ...
  if (str.len == 0) {
    struct session *sesh;
    vec_foreach(sesh, m->sessions) {
      if (sesh->input == src->fd) {
        app_detach_session(m, sesh);
        return;
      }
    }
  }
...
static int create_socket() {
  ...
  logmsg("Created socket at %d %s", sockfd, addr.sun_path);
  return sockfd;
}
...
static void draw_no_mans_land(struct app_context *app) {
  ...
  vec_foreach(sesh, app->sessions) {
    if (sesh->ws.colums && sesh->ws.rows) {
      ...
      // string_push_csi(&scratch, 0, INT_SLICE(38, 2, 255, 0, 0), u8"m");
