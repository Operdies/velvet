...
static lua_Integer luaL_shift(lua_State *L, lua_stackIndex idx, lua_stackIndex argtop) {
  assert(argtop > 0);
  if (idx == -1) idx = lua_absindex(L, idx);
  assert(idx > argtop);
  lua_pushvalue(L, idx);
  lua_insert(L, argtop);
  return argtop;
...
/* Insert stack item `idx` at index `argtop` and shift everything above argtop.
 * This is useful in the autogenerated marshalling code for values which cannot
 * be immediately represented as a C primitive. The main use case is functions passed in table parameters.
 * When the autogenerated code walks tables, it can read out strings and integer values from lua and store them
 * as C primitives, but function pointers must be stored as handles. That handle is normally a stack index,
 * which is later pinned in a lua registry via lua_ref().
 * But while walking tables, it is not possible to get a reliable stack index.
 * So instead, the marshalling code uses the bottom of the stack space above
 * the normal input parameters as a scratch space for temporarily pinning references.
 * */
lua_Integer luaL_checkfunction_and_shift(lua_State *L, lua_stackIndex idx, lua_stackIndex argtop) {
  luaL_checktype(L, idx, LUA_TFUNCTION);
  return luaL_shift(L, idx, argtop);
}
