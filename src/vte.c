...
static void ground_accept(struct vte *vte) {
  struct utf8 clear = {0};
  struct screen *g = vte_get_current_screen(vte);
  struct screen_cell c = { .symbol = vte->pending_symbol, .style = vte_get_current_screen(vte)->cursor.brush };
...
static void ground_reject(struct vte *vte) {
  ...
  struct screen_cell replacement = {.symbol = utf8_fffd};
  struct screen *g = vte_get_current_screen(vte);
  screen_insert(g, replacement, vte->options.auto_wrap_mode);
  uint8_t n = utf8_length(copy);
  struct string_slice s = { .len = n - 1, .content = &copy.utf8[1] };
...
static void vte_dispatch_csi(struct vte *vte, uint8_t ch) {
  ...
  if (ch >= 0x40 && ch <= 0x7E) {
    ...
    int parsed = csi_parse(&csi, (struct string_slice) { .len = len, .content = buffer });
...
static void vte_dispatch_osc(struct vte *vte, uint8_t ch) {
  ...
  if (ch == BELL || (ch == '\\' && prev == ESC)) {
    ...
    struct osc osc = {0};
    osc_parse(&osc, (struct string_slice) { .len = len, .content = buffer }, (uint8_t*)st);
...
static int consume_printables(struct vte *vte, size_t i, struct string_slice str) {
  struct screen *s = vte_get_current_screen(vte);
...
void vte_process(struct vte *vte, struct string_slice str) {
  assert(vte->rows);
  assert(vte->columns);
