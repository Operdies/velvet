...
static void velvet_scene_render_set_cell(struct velvet_scene_render *r, int line, int column, struct screen_cell value) {
  struct velvet_scene_render_line *l = &r->lines[line];
...
static void velvet_scene_render_set_cursor(struct velvet_scene_render *r, struct cursor_options cursor) {
...
static void velvet_scene_render_set_style(struct velvet_scene_render *r, struct screen_cell_style style);
...
void velvet_scene_render(struct velvet_scene *m, render_func_t *render_func, bool full_draw, void *context) {
  ...
  struct pty_host *h;
  struct velvet_scene_render *r = &m->render;
  ...
  for (int i = 0; i < r->h; i++) {
    struct velvet_scene_render_line *l = &r->lines[i];
  ...
  vec_foreach(h, m->hosts) {
    ...
    for (int line = 0; line < active->h; line++) {
      ...
      for (int column = 0; column < active->w; column++) {
        ...
        velvet_scene_render_set_cell(r, render_line, render_column, screen_line->cells[column]);
      }
    }
  }
  ...
  for (int line = 0; line < r->h; line++) {
    ...
    for (int col = l->damage.start; col <= l->damage.end; col++) {
      ...
      velvet_scene_render_set_style(r, c->style);
...

  {
    ...
    velvet_scene_render_set_cursor(r, focused->emulator.options.cursor);
  }
...
static void sgr_color_apply(struct velvet_scene_render_sgr_buffer *sgr, struct color col, bool fg) {
