    ...
    if (attribute->primary == 0) {
      ...
      if (color == 8) {
        ...
        if (type == 5) { /* color from 256 color table */
          ...
          *target = (struct color){.c.table = color, .kind = VELVET_API_COLOR_KIND_TABLE};
        } else if (type == 2) {
          ...
          *target = (struct color){.c.rgb = {.r = red, .g = green, .b = blue}, .kind = VELVET_API_COLOR_KIND_RGB};
        }
      } else if (color == 9) { /* reset */
        *target = color_default;
      } else { /* This is a normal indexed color from 0-8 in the table */
        *target = (struct color){.c.table = attribute->primary % 10, .kind = VELVET_API_COLOR_KIND_TABLE};
      }
    } else if (attribute->primary >= 90 && attribute->primary <= 97) {
      int bright = 8 + attribute->primary - 90;
      style->fg = (struct color){.c.table = bright, .kind = VELVET_API_COLOR_KIND_TABLE};
    } else if (attribute->primary >= 100 && attribute->primary <= 107) {
      int bright = 8 + attribute->primary - 100;
      style->bg = (struct color){.c.table = bright, .kind = VELVET_API_COLOR_KIND_TABLE};
    }
  }
  return NULL;
}
...
bool csi_dispatch(struct vte *vte, struct csi *csi) {
  ...
  for (int i = 0; i < csi->n_params; i++) {
    if (csi->params[i].primary < 0) {
      ...
      velvet_log("Negative csi encountered: %.*s", (int)vte->command_buffer.len, vte->command_buffer.content);
      return false;
    }
  }
...
static bool REP(struct vte *vte, struct csi *csi) {
  struct screen *g = vte_get_current_screen(vte);
  int count = csi->params[0].primary ? csi->params[0].primary : 1;
  struct screen_cell repeat = { .cp = vte->previous_symbol, .style = g->cursor.brush, .link = vte->current_link };
...
static bool SGR(struct vte *vte, struct csi *csi) {
  ...
  if (error) {
    velvet_log("Error parsing SGR: %.*s: %s", (int)vte->command_buffer.len - 1, vte->command_buffer.content + 1, error);
    return false;
  }
