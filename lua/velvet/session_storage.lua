...
-- session storage, but it is not clear what the use case would be.
-- For now, we will treat unreferenced keys as though they don't exist.
local known_stores = {}

-- write all stores on reload.
local e = vv.events.create_group("velvet.session_storage", true)
e.pre_reload = function()
  for k, v in pairs(known_stores) do
    ...
    pcall(vv.api.session_store_value, k, v)
  end
end

return {
  --- Create a new session storage. Values written to this object
  ...
  --- @return table storage A storage table which can be used to access session-persisted values.
  create = function(key)
    if not known_stores[key] then
      known_stores[key] = vv.api.session_load_value(key) or {}
    end
