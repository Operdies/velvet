...
local function run_segment(name, update_completed, bar_updating, elem_data, elem_options)
  ...
  while registry[name] do
    ...
    for i, v in ipairs(update_triggers) do
      ...
      if type(v) == 'number' and v < 100 then update_triggers[i] = 100 end
    end
...
local function bar_dispatch_events(bar)
  local priv = bar_data[bar]
  local win = priv.win
  local bar_updating = priv.bar_updating
  local draw = false
  local polling = false
  local poll_max = 10
  ...
  for _ = 1, poll_max do
    local event, event_data = vv.async.wait(
      ...
      -- if any segment was updated, keep polling with a tiny timeout before updating the bar. This lets us update everything in one go.
      -- if no segment was updated, just poll without a timeout
      polling and 0 or nil
    )
    ...
    elseif event == priv.update then
      draw = true
      -- we want to continue polling (now with a small timeout)
      -- after emitting the update event. This allows segments
      -- with no update triggers to finish updating.
      if not polling then
        -- Guard against recursing bar_updating events. This can happen when an element without any triggers runs async.
        local trigger = event_data.data
        if trigger ~= nil and trigger ~= bar_updating:listener() then
          bar_updating:emit()
        end
        polling  = true
      end
    ...
    -- continue flushing only if the event is priv.update
    if event ~= priv.update then break end
  end
